...
class TestGetTipsAndTaxaNamesAndTaxaCountsFromSubtrees(object):
    def test_get_all_tips_and_taxa_names(self):
        ...
        expected_counts_of_taxa_from_terms = Counter(
            {
                "Aspergillus_awamori_IFM_58123": 10,
                "Aspergillus_niger_CBS_513.88": 9,
                "Aspergillus_fumigatus_Af293": 8,
                "Aspergillus_oerlinghausenensis_CBS139183": 8,
                "Aspergillus_fischeri_NRRL181": 6,
            }
        )
    ...
    def test_get_subtree_tips(self):
        ...
        for inter in tree.get_nonterminals()[1:]:
            (subtree_tips, dups) = get_subtree_tips(
                terms, "Aspergillus_fumigatus_Af293", inter
            )
            break
...
class TestKeepLongSequences(object):
    def test_keep_long_sequences(self):
        ...
        for inter in tree.get_nonterminals()[1:]:
            (newtree, terms) = keep_long_sequences(inter, fasta_dict, dups, terms)
            break
